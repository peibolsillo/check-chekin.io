...
STATE_FILE    = "chekin_state.json"
REPORT_FILE   = "chekin_report.json"
TOKENS_FILE   = "chekin_tokens.json"
HISTORY_FILE  = "chekin_history.json"     # log acumulativo cambios
INTERVAL_MIN  = 30

# SMTP Gmail — notificación al terminar consulta
...
def get_token_via_browser(email: str, password: str) -> dict:
    ...
    # En servidor sin display (HEADLESS=true) no se puede mostrar login interactivo
    if HEADLESS:
        raise RuntimeError(
            "HEADLESS=true: sin display para login interactivo. "
            "Captura tokens en máquina local (HEADLESS=false), "
            "luego copia chekin_tokens.json al servidor."
        )
        ...
        try:
            ...
            while time.time() < deadline:
                if captured["access_token"]:
                    break
                page.wait_for_timeout(250)  # poll rápido → cierre inmediato tras captura
            ...
            # Copiar cookies antes de cerrar
            try:
                cookies = [
            ...
            # Cerrar siempre — context primero, browser después
            try:
                context.close()
            except Exception:
                pass
...
def run_check():
    ...
    try:
        ...
        log.info(f"  → {len(raw_reservations)} reservas encontradas.")
